...
任务来源为成功脚本复刻或脚本ID以`wsr_`开头时，在付费前消费人物模板合同中的`reference_assets`角色索引，校验与完整附件列表一致。人物/商品角色说明只加入内存执行prompt，不改飞书正文。

人物只定义脸部身份、商品定义假发外观的共用说明由执行层补一次，不要求正文包含特定措辞。修订/对照测试的原母版点、有效检查点和允许变化随合同及trace保留；测试时按有效要求判断，不能把已允许改变的拍法当违约。此元数据不进入画面指令，不触发生产自动检查。

此来源仅放行即梦“全能参考”最多12图或MiniMax H3“多参考图”最多9图。iMini、首尾帧、超限、缺失或错序合同明确阻塞，不截断、不自动换渠道。无图任务需显式免参考图；就绪且已选择的统一首帧按合成图角色传递。其他来源沿用原规则。

新提单要求人物模板合同 `schema_version=2`，每张图绑定 `file_token / original_sha256 / derived_sha256`；运行表实际附件 token、顺序和 manifest 身份必须一致。旧 v1 新任务需重新同步升级；已提交、已回传任务不重走提交。H3 以 `derived_sha256` 校验实际下载字节，完整解码为真实图片后上传；保留原图身份 hash 与上传字节 hash，二者不混用。

H3 在付费提单前把最终执行提示词、完整请求、有序图片角色/路径/三个 hash 和 `mm_file` 映射保存到机器级 trace。参考图缓存位于 `_state/minimax-h3/reference-cache`，默认只复用本地图片字节，每个新视频重新上传并记录远端映射；远端素材句柄生命周期尚未验证，`referenceUploadCacheTtlMs` 默认 `0`（关闭），仅显式配置才启用限时复用。已知旧正文“前两张人物、后两张产品”索引只在执行副本改成实际图号，保留人工表格正文。

成片内容检查仅用于用户明确要求的测试阶段：保留独立 `scripts/run_wsr_content_review.py`（默认不调用模型；明确授权指定视频、参考图和提示词外发后才使用 `--apply-model`）。生产 worker 的提单、成片回传、恢复补写均不调用检查模型；旧 `contentReviewEnabled` 配置不再触发自动检查。`内容检查结果`（通过/有偏差/检查异常）与 `内容检查说明` 两字段保留，仅显式 `--ensure-schema-only` 时创建，可由测试流程回填。检查不修改生成/发布状态，不自动重生成或发布，不增加巡检频率。
