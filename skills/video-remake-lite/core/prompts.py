...
时长决策矩阵：
- 原视频 < 6 秒：复刻策略 = 短片延展复刻（目标时长 = 下游最短支持档；保留原主动作，只在动作前增加自然起势/微停顿、动作后增加自然回收，不循环、不慢放、不复制帧）
...
13. 如果 Step1 判定为「短片延展复刻」：总时长必须等于 Step1 的下游最短支持档；原视频主动作必须保留为唯一高光，只能在动作前增加自然准备/视线先行/微停顿、在动作后增加自然回收/呼吸/视线收尾，禁止循环、慢放、复制帧或新增第二主动作。
14. 如果 Step1 判定为「选段压缩复刻」：仅在 Step1 选定的高光段范围内改写，禁止把【被舍弃段落】塞回复刻方案；目标时长固定为 15 秒（±1秒）
15. 如果 Step1 判定为「不建议复刻」：本步骤不应该被触发；如确实被触发，请直接在「一、复刻策略」中说明并停止输出
...
- 若是短片延展复刻，标出原主动作时间区间，并在补足部分标注“生成补足：自然起势/微停顿/动作回收”；禁止循环、慢放、复制帧或新增第二主动作
- 第1段（0-?秒）：
- 第2段（?-?秒）：
- 第3段（?-?秒）：
- 第4段（?-?秒）：
...
- 镜头编号
- 时长（秒，精确到 0.5 秒）
- 对应原视频时间（若 Step1 决策为「选段压缩复刻」则填具体区间如「8.0-11.5s」；若为「短片延展复刻」则注明原主动作区间或“生成补足：自然起势/动作回收”；若为「原长复刻」则填「原长复刻」）
- 场景
- 人物/动作
- 情绪目标
- 镜头内容
