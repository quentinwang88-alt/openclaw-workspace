...
- `场景图Prompt`: exact S1-S4 scene prompt package sent to image generation.
...
- `场景图槽位`: optional multi select. Blank means S1-S4; selected values can include `S1 主点击试穿`, `S2 日常氛围`, `S3 版型/颜色证明`, `S4 材质结构细节`.
...
- `反馈目标图`: multi-select, which image(s) to fix (`首图`, `S1`-`S6`).
