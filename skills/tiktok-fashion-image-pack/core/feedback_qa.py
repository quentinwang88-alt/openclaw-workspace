...
2. 检查图片 1 的材质、版型/尺寸比例、扣子、口袋、领型、袖口、下摆、发饰固定结构，以及假发的发根色、渐变、长度、卷度、分缝、发际线、帽网结构是否与图片 3 起的原始参考一致
...
7. 如果反馈提到扣子、口袋、领型、袖口、下摆、发夹夹齿、发箍、蝴蝶结、珍珠/水钻、颜色、数量等结构问题，图片 1 中该结构必须清晰可见；如果被手、袖子、头发、角度遮住导致无法确认，应判为"部分通过"或"不通过"
...
{{
  ...
  "score": 0.0 到 1.0 之间的分数,
    ...
    {{
      "issue": "反馈中的具体问题描述",
      "category": "结构未修正" 或 "结构被遮挡" 或 "数量不精确" 或 "疑似镜像错误" 或 "颜色跑偏" 或 "发饰细节错误" 或 "假发细节错误" 或 "风格可用但细节需人工看" 或 "其它",
...
def classify_feedback_qa_issue(issue: str, detail: str) -> str:
    ...
    if any(word in text for word in ("镜像", "相反", "左侧", "右侧", "左右")):
        return "疑似镜像错误"
    if any(word in text for word in ("发饰", "发夹", "鲨鱼夹", "发箍", "发圈", "蝴蝶结", "夹齿", "扣齿", "珍珠", "水钻", "头饰")):
        return "发饰细节错误"
    if any(word in text.lower() for word in ("假发", "发际线", "分缝", "帽网", "蕾丝", "lace", "卷度", "发根", "渐变", "刘海", "发尾")):
        return "假发细节错误"
