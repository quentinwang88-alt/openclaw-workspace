...
FIELD_SPECS: List[Dict[str, Any]] = [
    ...
    {"name": "商品链接/供应商链接", **URL},
    {"name": "国家", **single_select(["TH", "VN", "MX", "MY", "PH", "ID", "SG"])},
    {"name": "类目", **single_select(["女装上装/外套", "发饰", "假发", "配饰", "包袋", "鞋靴"])},
    ...
    {"name": "袖型/袖口", **TEXT},
    {"name": "下摆结构", **TEXT},
    # Wig-specific truth fields
    {"name": "假发产品形态", **single_select(["full_wig", "ponytail_piece", "clip_in_extension", "hair_topper", "unknown"])},
    {"name": "发丝材质", **TEXT},
    {"name": "帽网结构", **TEXT},
    {"name": "Lace范围", **TEXT},
    {"name": "发际线/分缝", **TEXT},
    ...
    {"name": "场景图质检问题", **TEXT},
    {"name": "场景图生成明细", **TEXT},
    {"name": "场景偏好", **single_select(["自动匹配", "咖啡店/商场", "通勤街边", "镜前试穿", "店铺/工作室", "现代梳妆台/家居", "墨西哥城市日常", "旅行/轻户外", "校园/日常"])},
    ...
    {"name": "图片反馈问题", **TEXT},
    {"name": "反馈状态", **single_select(["待修正", "修正中", "已修正", "需人工复核"])},
    {"name": "反馈处理方式", **single_select(["局部修正", "整图重生"])},
    {"name": "反馈修正结果", **ATTACHMENT},
    {"name": "反馈修正结果_场景图", **ATTACHMENT},
    {"name": "反馈修正Prompt", **TEXT},
    {"name": "反馈质检结果", **single_select(["通过", "轻微问题可用", "不通过", "需人工复核"])},
    {"name": "反馈质检问题", **TEXT},
]
