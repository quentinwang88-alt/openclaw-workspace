...
下装颜色不需要在原始脚本表逐项选择。系统会在每个视觉方案中自动抽取一个颜色并写入“实际下装颜色”；同一方案重跑以及生成 1 条或 5 条视频时保持不变。搭配模板的“下装颜色”只作为可选候选池，留空时按下装类型使用系统默认色池。多个“下装版型”标签是同一件下装的累计要求，不会随机组合或拆成多个方案。`适配产品编码` 用于向原创脚本共享结构化穿搭：精确编码优先，`*` 为通用，留空不共享；原创流程不读取搭配名称、搭配核心描述或备注。
...
飞书维护人设、账号品牌视觉、场景环境、镜头方案、动作、搭配、字幕和产品视觉方案复核；SQLite 仍是商品、方案、任务、生成、后处理、QC 的执行真相源。搭配表的 `适配场景族` 使用中文多选（居家日常、咖啡/品质室内、街头/外出、镜前/试穿、办公/通勤、乘车/等候），同步层会在飞书中文值与内部稳定代码之间双向转换。账号品牌 Logo 会在模板同步时缓存到本地，后期精确叠加，不交给视频模型生成。模板同步后会在同一轮自动刷新原始脚本表的场景、搭配、动作和镜头方案名称选项。选填的场景参考图会缓存到本地并进入穿搭图请求；为空时使用场景文字。下装颜色由系统自动解析，不增加原始脚本操作字段；产品视觉方案表中的“实际下装颜色/实际下装版型”仅用于查看结果。新增场景只需在场景环境库增加一条环境记录，不要为全身、半身、推近重复建场景。
