...
保存 Beat 缺口所需的补充镜头、完整提示词、参考图、质检、重试次数和降级方式。单条增强型视频按实际证据缺口生成，硬上限 3 个；没有缺口时不为凑数量生成。
...
Prompt 由模板编译，不自由生成。每条都必须独立写全场景、人物、商品、内搭、下装、构图、时间动作和负面限制。闭合方式、领口、腰线、颜色和面料类口播必须分别命中对应角色；多色镜头没有可靠参考图时降级为真实商品图片轮播。
...
→ waiting_tts
→ matching_assets
→ voiceover_cut_planned
→ waiting_supplement_assets / ready_to_mix
→ rendering
→ final_qc
→ review_ready
```
...
→ 逐条口播匹配证据镜头并按真实台词时间重排画面
→ 缺镜头时最多补 3 个，回传后重新打标与重排
→ 连续口播音轨与重排画面合成
→ final_qc
```

粗剪只消费 `tag_status=completed` 且 `asset_status=ready` 的实际切片；生成前计划标签不得充当证据。3秒切片会按目标时长自适应组合，并在帧尾不足时补齐到精确目标时长。18 秒以上口播全文只请求一次 TTS，禁止逐句拼音频；增强型泰语默认采用自然偏慢的 `-18%` 语速，让 22 秒视频保持更高口播占用率。系统优先使用服务返回的词级边界对齐 Beat，服务缺失边界时才按句子权重回退估算。每条台词按 `required_shot_roles` 匹配实际素材，找不到对应镜头时形成 `evidence_gap` 并阻断混音。TTS 超时必须回到当前口播流程精简，不由增强层改写或强制加速。最终混音默认 `voiceover_only_no_random_model_bgm`，BGM 需后续建立口播专用压低/闪避策略后再启用。
...
- 每条具体卖点口播必须命中相应镜头角色；必要 Beat 缺口最多触发 3 个补镜头。
- 同一素材文件和标签体系版本只打标一次。
- 实际标签覆盖计划标签时，以实际标签为准。
