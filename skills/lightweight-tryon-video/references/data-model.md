...
存储适配产品编码、适用上装/商品类型、下装、颜色、版型、配饰、风格和 Prompt，并提供选填的内搭类型、内搭颜色、内搭补充要求。`适配产品编码` 是跨流程的结构化授权字段：可用逗号、中文逗号、分号或换行填写多个编码，`*` 表示通用，留空表示模板只供轻视频自身使用。原创脚本共享 Provider 只读取编码、商品类型、版型、内搭、下装、配饰、鞋履、风格和版本等结构化字段，不读取搭配名称、核心描述、备注等正文。外套类优先使用模板内搭值，空字段由系统自动补齐；STYLE_006 对套装、连衣裙和连体款有保护分支，避免凭空添加下装。
