...
description: |
  飞书脚本总库送生产与分流系统的实现、开发和高级故障诊断 skill。新“原创视频生产脚本”按视频形态把15秒原创交给短视频运行管理表、20–45秒原创长视频交给 Plan C、长复刻交给复刻分段执行器；旧 S1–S4 表保留历史兼容。日常自然语言检查、生成勾选视频、续跑长视频和发布已确认长视频优先使用 original-content-operator；需要修改同步、幂等、参考图交接或生产路由时使用本 skill。
