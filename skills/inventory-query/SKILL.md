...
5. **前台名称回退查询**：如果按实际库存编码 `sku` 查不到，系统会优先尝试本地别名映射，再尝试按前台展示名称 `title` 查询。例如前台显示 `F-002`，实际库存编码可能是 `FSL001`
6. **别名映射文件**：可在 `title_sku_aliases.json` 中维护前台展示名到真实库存编码的映射，适合处理 `title != sku` 且接口本身不支持直接按 title 命中的场景
7. **字段命名约定**：对外展示时，原 `reserved` / “预留库存”统一解释为“仓库锁定待发库存”，表示仓库已锁定、尚未发出的订单占用库存；日常判断是否还能卖，优先看 `available`（可用库存）
