...
5. **前台名称回退查询**：如果按实际库存编码 `sku` 查不到，系统会先查本地已知的 `title -> sku` 映射，再尝试按前台展示名称 `title` 查询，最后才返回“找不到”。例如前台显示 `F-002`，实际库存编码可能是 `FSL001`
6. **别名映射自动沉淀**：每次成功查到 `sku/title` 后，系统会自动把 `title -> sku` 记入 `title_sku_aliases.json`，后续名称查询可直接回退命中
7. **补货报告别名兜底**：启动时会顺带扫描 `skills/output/*.md` 里的补货报告标题（形如 `### SKU - TITLE`），把历史报告里的名称映射也纳入回退范围
8. **字段命名约定**：对外展示时，原 `reserved` / “预留库存”统一解释为“仓库锁定待发库存”，表示仓库已锁定、尚未发出的订单占用库存；日常判断是否还能卖，优先看 `available`（可用库存）
