...
总库新来源通过`app/script_pool.py`登记稳定脚本ID、用途、挂车和图片来源等元数据；同步失败可重试，已排期/提交内容冻结。用途与是否挂车独立：养号默认不挂车，可明确覆盖；原种草/非商品展示专线的不挂车限制保留。

内部产品编码与平台商品ID分离；`S...`不能当挂车ID。挂车ID缺失/非法时禁止发布而非阻止脚本入库；不挂车时同时清理平台请求中的历史productId/productTitle。店铺、标题和账号能力仍由现有发布准备流程校验。

墨西哥/MX/Mexico/México/es-MX及MX店铺前缀走西语标题流程；中文管理标题、空标题或英文兜底不能作合格西语发布标题。登记脚本不调用模型、不生成视频、不自动排期或发布。
...
- 固定发布时间槽
- 未来 48 小时窗口，避免 48 小时内容家族规则导致当天槽位空掉后无法自动顺延
- 每次只补未排期槽位
