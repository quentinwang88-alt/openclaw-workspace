...
REPORT_FIELDS: Sequence[Dict[str, Any]] = (
    ...
    {"name": "实际发布素材", "type": 1, "ui_type": "Text"},
...
def build_report_fields(row: Any) -> Dict[str, Any]:
    ...
    source_type = _normalize_text(row["video_source_type"])
    if source_type == "voiceover_attachment":
        publish_material = "口播成片"
    elif source_type:
        publish_material = "生成视频"
    else:
        publish_material = ""
    return {
...
def sync_publish_report_table(db: AutoPublishDB, client: Any) -> Dict[str, int]:
    ...
    for record in client.list_records(page_size=500):
        ...
        if canonical_script_key and canonical_script_key not in existing_map:
            existing_map[canonical_script_key] = record
        if script_id and script_id not in existing_map:
            existing_map[script_id] = record
    ...
    for row in rows:
        ...
        existing = existing_map.get(canonical_script_key) or existing_map.get(script_id)
        if existing:
            changed_fields = {
                field_name: value
                for field_name, value in fields.items()
                if _field_text(existing.fields.get(field_name)) != _field_text(value)
            }
            if changed_fields:
                updates.append({"record_id": existing.record_id, "fields": changed_fields})
            else:
                unchanged += 1
        else:
