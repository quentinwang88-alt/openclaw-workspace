...
def _frame_safe_saliency(world: Mapping[str, Any]) -> Dict[str, Any]:
    saliency = dict(world.get("visual_saliency") or {})
    separation = dict(saliency.get("separation") or {})
    if text(dict(world.get("outfit_prompt_projection") or {}).get("frozen_outfit")):
        separation["outfit_guidance"] = (
            "冻结穿搭保持不变；如需增强商品边界，只调整背景明暗、人物站位或光线方向，"
            "不得改变内搭、下装、鞋履或配饰。"
        )
    if separation:
        saliency["separation"] = separation
    return saliency
...
def _base_frame_contract(master: Mapping[str, Any], visual: str, action: str,
    ...
    scene_contract = dict(world.get("scene_contract") or {"description": world.get("scene")})
    if scene_block:
        scene_contract.update({key: value for key, value in dict(scene_block).items() if value})
    return {
        ...
        "visual_saliency": _frame_safe_saliency(world),
        "opening_scene_projection": dict(world.get("opening_scene_projection") or {}),
        "opening_contract": {
            "visual_content": visual,
            "character_action": action,
            "natural_emotion": text(world.get("natural_emotion")) or "自然、正在分享，不做广告式表演",
            "camera": text(world.get("camera")) or "普通手机竖屏平视构图",
            "carrier_mode": text(world.get("carrier_mode")) or "PERSON_ON_CAMERA",
            "product_anchors_visible": anchors,
        },
        "presentation_mode": text(world.get("presentation_mode")) or "PERSON_ON_CAMERA",
...
def build_keyframe_contracts(master: Mapping[str, Any], plan: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    first_unit = dict((segments[0].get("capture_units") or [])[0])
    ...
    k0_prompt = render_first_frame_prompt(k0_contract, frame_role="OPENING")
    result = {
        "schema_version": "longform-keyframe-package-v5-boundary-aware",
    ...
    for index, boundary in enumerate(boundaries, 1):
        ...
        if boundary_mode == "DISCONTINUOUS_CUT":
            first_unit = dict((next_segment.get("capture_units") or [])[0])
            ...
            result[entry_key] = {
                "role": "SCENE_ENTRY",
                "prompt": render_first_frame_prompt(entry_contract, frame_role="SCENE_ENTRY"),
                "source": "SYSTEM_GENERATED",
                "generation_strategy": "NEW_SCENE_WITH_FROZEN_PERSON_PRODUCT_OUTFIT",
                "reference_policy": {
                    "primary": "PRODUCT_AND_APPROVED_PERSONA_REFERENCES",
                    "continuity_locks": ["SAME_PERSONA", "SAME_PRODUCT", "SAME_OUTFIT"],
                    "scene_authority": "TARGET_SCENE_BLOCK",
                },
        ...
        bridge_unit = dict((segment.get("capture_units") or [])[-1])
