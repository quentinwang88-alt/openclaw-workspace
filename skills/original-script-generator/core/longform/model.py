...
4. 每个拍摄单元都应有可自然停住的结束状态；只有下一段确实延续同一物理动作时才要求从尾帧继续，不为桥接增加摆拍。
...
只返回JSON。保留输入的所有顶层权威字段，并新增 scene_blocks 与 capture_units。片段ID固定为{segment_ids}。scene_blocks最多2个，每个包含 scene_id、location、moment、lighting、background、narrative_role、segment_ids；一个片段只能属于一个场景。每个场景的segment_ids必须连续，全片最多出现一次场景切换，例如A在场景1、B/C在场景2，或A/B在场景1、C在场景2；禁止A/C同场景而B为另一场景。每个capture_unit至少包含 unit_id、segment_id、scene_id、beat、visual_content、information_gain、camera、character_action、product_anchors_visible、execution_priority；可自然停住时补充 observable_end_state 或 end_state。
