#!/usr/bin/env python3
"""Small SQLite/MySQL compatibility helpers for original-script storage."""
...
class MySQLCompatConnection:
    ...
    def _replace_tables(self, sql: str) -> str:
        ...
        for name in self._table_names:
            text = re.sub(r"(?<![A-Za-z0-9_`]){name}(?![A-Za-z0-9_`])".format(name=re.escape(name)), self._prefix + name, text)
        return text
    ...
    def _rewrite_upsert(self, sql: str) -> str:
        ...
        if not match:
            return sql
        update_part = re.sub(r"\bexcluded\.([A-Za-z_][A-Za-z0-9_]*)", r"VALUES(\1)", sql[match.end() :])
...
def resolve_database_url(value: Any, env_name: str) -> str:
    ...
    if raw.startswith("mysql://") or raw.startswith("mysql+pymysql://"):
        return raw
    return (os.environ.get(env_name) or os.environ.get("LIKEU_AI_DATABASE_URL") or "").strip()
