...
EAR_ACCESSORY_KEYWORDS = {"耳环", "耳饰", "耳钉", "耳夹", "耳坠"}
...
5. 不允许 4 套都使用同一种 opening_mode；
6. 不允许 4 套都使用同一种 ending_mode；
7. 至少出现 3 种不同的 proof_mode；
8. 至少出现 3 种不同的 visual_entry_mode；
9. 至少 2 种不同的 persona_state；
10. 至少 3 种不同的 action_entry_mode；
11. 4 条脚本的 rhythm_signature 不允许完全相同；
12. S4 与 S1 的差异不能只是“更好看一点”；
13. 如果 target_country 属于东南亚市场：
    - 至少 2 套以家中自然分享场景为主
    - 家中场景至少覆盖 2 种 scene_subspace
    - 不允许 4 条都落在同一个窗边 / 同一面镜子 / 同一机位逻辑里
    - S4 仍应优先保留家中自然分享语境
14. 不允许 4 条全部使用同一个 styling_completion_tag；
15. 至少出现 2 种不同的 styling_completion_tag；
16. 不允许 4 条全部使用同一个 persona_visual_tone；
17. 至少出现 2 种不同的 persona_visual_tone；
18. 不允许 4 条全部使用同一个 emotion_arc_tag；
19. 至少出现 2 种不同的 emotion_arc_tag；
20. styling_key_anchor 允许少量重复，但不要 4 条都完全一样；
21. 这些差异必须是轻差异，不得把人物写成明显不同的人设；
22. 差异服务于内容感知分化，不服务于抢商品注意力。
...
10. 至少 2 种不同的 styling_completion_tag；
11. 至少 2 种不同的 persona_visual_tone；
12. 至少 2 种不同的 emotion_arc_tag；
13. styling_key_anchor 不允许 4 条完全相同；
14. 若 opening/proof/ending 已有差异，但人物视觉感、穿搭完成感、情绪轨迹仍然像同一条模板，应视为感知近似度仍偏高，需要轻修正。
...
12. 不要输出大段“为什么这样设计”的解释。
13. 脚本必须严格继承 styling_completion_tag / persona_visual_tone / styling_key_anchor / emotion_arc_tag；
14. 穿搭表述不能只写“低饱和纯色、不抢商品”，还要体现当前的 styling_completion_tag；
15. 人物状态不能只写“自然、轻笑、顺手分享”，还要体现当前的 persona_visual_tone；
16. styling_key_anchor 必须落实到执行约束里，说明哪一个穿搭视觉点最关键；
17. emotion_arc_tag 必须落实到人物动作与表情推进中；
18. 上述字段都只负责轻度拉开气质，不允许压过商品展示。
19. 如果 script_brief.parameter_anchors 非空，脚本必须保持这些参数事实，不得改写、偷换或与镜头设计冲突；
20. parameter_anchors 不要求逐条口播，但关键参数至少要做到“画面可见、表达不冲突、结果不被写歪”。
...
8. 人物状态是否符合 persona_state
9. 情绪推进是否成立
10. 穿着是否符合要求，是否抢商品
11. 配饰类是否完整露耳、无遮挡关键结构
12. 是否存在明显模板化表达
13. 当前脚本的人物视觉感是否符合 persona_visual_tone
14. 当前脚本的穿搭描述是否只停留在“避免错误”，还是已经体现 styling_completion_tag
15. 当前脚本是否明确落实了 styling_key_anchor
16. 当前脚本的情绪推进是否符合 emotion_arc_tag
17. 是否存在“人物看起来仍像同一批模板人、同一类安全穿搭、同一种平情绪轨迹”的问题
18. 如果商品锚点卡里存在 parameter_anchors，当前脚本是否错误改写、忽略关键参数，或让参数事实与画面/口播冲突
