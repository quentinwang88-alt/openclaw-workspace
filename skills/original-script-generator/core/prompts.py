...
def _compact_q1_context(
    ...
    if not isinstance(human_contract, dict):
        human_contract = {}
    scene_seed_brief = human_contract.get("scene_seed_brief") if isinstance(human_contract.get("scene_seed_brief"), dict) else {}
    strategy_id = str(final_strategy_json.get("strategy_id", "") or "").strip().upper()
    scene_strategy_map = (
        scene_seed_brief.get("strategy_by_script_role")
        if isinstance(scene_seed_brief.get("strategy_by_script_role"), dict)
        else {}
    )
    scene_strategy = (
        scene_strategy_map.get(strategy_id)
        if isinstance(scene_strategy_map.get(strategy_id), dict)
        else {}
    )
    ...
    return {
        ...
        "scene_seed": pre_qc_result.get("scene_seed", {}) if isinstance(pre_qc_result, dict) else {},
        "scene_seed_brief": {
            "enabled": bool(scene_seed_brief.get("enabled")),
            "current_script_strategy": {
                "strategy_id": strategy_id,
                "seed_mode": scene_strategy.get("seed_mode", ""),
                "moment_bias": scene_strategy.get("moment_bias", ""),
                "tension_bias": scene_strategy.get("tension_bias", ""),
                "camera_gaze_bias": scene_strategy.get("camera_gaze_bias", ""),
                "payoff_bias": scene_strategy.get("payoff_bias", ""),
            },
        },
        "scene_seed_liveliness_check": (
            pre_qc_result.get("scene_seed_liveliness_check", {}) if isinstance(pre_qc_result, dict) else {}
        ),
...
def _compact_script_brief_for_p7(script_brief_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    scene_seed_strategy_map = (
        scene_seed_brief.get("strategy_by_script_role")
        if isinstance(scene_seed_brief.get("strategy_by_script_role"), dict)
        else {}
    )
    scene_seed_strategy = (
        scene_seed_strategy_map.get(strategy_id)
        if isinstance(scene_seed_strategy_map.get(strategy_id), dict)
        else {}
    )
    ...
    compact = {
        ...
        "scene_seed_brief": {
            "enabled": bool(scene_seed_brief.get("enabled")),
            "display_family": scene_seed_brief.get("display_family", ""),
            "seed_goal": _short_text(scene_seed_brief.get("seed_goal"), 120),
            "current_script_strategy": {
                "strategy_id": strategy_id,
                "seed_mode": _short_text(scene_seed_strategy.get("seed_mode"), 60),
                "moment_bias": _short_text(scene_seed_strategy.get("moment_bias"), 90),
                "tension_bias": _short_text(scene_seed_strategy.get("tension_bias"), 90),
                "camera_gaze_bias": _short_text(scene_seed_strategy.get("camera_gaze_bias"), 90),
                "payoff_bias": _short_text(scene_seed_strategy.get("payoff_bias"), 90),
            },
            "moment_hints": _short_list(scene_seed_brief.get("moment_hints"), 4, 80),
            "small_tension_hints": _short_list(scene_seed_brief.get("small_tension_hints"), 5, 80),
            "micro_behavior_boundary": {
                "safe_behavior_hints": _short_list(
                    (scene_seed_brief.get("micro_behavior_boundary") or {}).get("safe_behavior_hints")
                    if isinstance(scene_seed_brief.get("micro_behavior_boundary"), dict)
                    else [],
                    5,
                    90,
                ),
                "risk_boundary": _short_list(
                    (scene_seed_brief.get("micro_behavior_boundary") or {}).get("risk_boundary")
                    if isinstance(scene_seed_brief.get("micro_behavior_boundary"), dict)
                    else [],
                    6,
                    90,
            ...
            "payoff_direction": _short_text(scene_seed_brief.get("payoff_direction"), 120),
            "anti_template_guidance": _short_list(scene_seed_brief.get("anti_template_guidance"), 4, 90),
        },
...
- scene_seed_brief：如果 performance_profile_json 中存在，则原样压缩传入。它只提供当前类目的生活动机短卡，不是固定场景库。
- scene_seed_brief.strategy_by_script_role：如果 performance_profile_json 中存在，必须原样传入；它只约束 S1-S4 的 scene_seed 差异方向，不是固定模板。
...
- 如果 script_json.scene_seed 存在，必须保留其中的生活片刻、轻微动机、小动作和确认感，并让最终提示词呈现镜头连续反应
