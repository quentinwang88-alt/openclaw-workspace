...
但包含商品身份、人物参考、人物比例、已解析穿搭结构、场景投影、视觉显著性、开场、模型与提示词版本。首帧提示词使用 `original-first-frame-prompt-v5-text-persona-fallback`：高密度场景只保留地点身份与一个侧边/远处锚点，不把书架、货架、文字、收据和道具动作同时塞入主体背后；真人方向把“正在开口说话”的动态脚本动作投影为“刚准备开口”的静止瞬间，嘴唇自然放松或仅轻微分开，并将人物参考图的脸部身份权威与结构化身体比例权威分开；视频口播本身不受影响。已明确绑定 `persona_id` 的真人方向仍必须具备批准人物参考图；未绑定人物模板、但完整脚本已经冻结一次性人物身份/外貌/妆发的方向，允许以 `FROZEN_SCRIPT_TEXT_ONLY` 生成，并明确禁止从商品参考图复制模特的脸、妆容、身材、姿势或构图，不能把它误判为缺少人物参考图。该投影只影响首帧与第一拍摄单元，不改完整脚本或后续片段场景。
