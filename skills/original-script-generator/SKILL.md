...
description: |
  原创脚本生成系统的实现、开发与高级故障诊断 skill，覆盖15秒原创和20–45秒原创长视频的商品锚点、卖点、结构、人物、穿搭、场景、中央口播、首帧及 Plan C 合同。日常自然语言检查、运行、续跑和送生产优先使用 original-content-operator；需要修改生成逻辑、分析批次结果或排查内部阶段时使用本 skill。旧 S1–S4 `run_pipeline.py` 只维护历史数据，禁止用于新任务或自然语言调度。
...
## OpenClaw 原创全链路自然语言入口

本 skill 是 OpenClaw 处理原创内容的主入口。先根据用户说的是“生成脚本”还是“把已确认脚本生成成片”选择阶段，不要临时拼 shell，也不要绕过飞书状态位。
...
| 用户自然语言 | action | 行为 |
| --- | --- | --- |
| 检查待执行原创任务 / 看看有没有待跑的原创长视频 | `check --limit 1` | 只读检查，不调用模型、不写表 |
| 跑一条原创任务 / 生成一条原创长视频脚本 | `run --limit 1` | 执行一行状态为“待执行”的运营任务，15 秒或长视频由该行视频规格决定 |
| 给产品 173... 生成原创内容 | `run --product-code 173...` | 只执行该产品唯一可执行任务；匹配多行时要求 record ID |
| 只规划某条任务 | `plan --record-id rec...` | 生成冻结计划，不写正式脚本 |
| 续跑失败任务 | `resume --record-id rec...` | 复用检查点续跑，不重做已完成阶段 |
| 更新输入或卖点后重做规划 | `replan --record-id rec...` | 新建规划批次，重新冻结来源 |
| 刷新原创人物和穿搭配置 | `refresh-production-config` | 刷新共享配置，不生成脚本 |

“生成一条原创长视频”里的“一条”默认表示最多处理一行运营任务，因此传 `--limit 1`；该行真正生成几条脚本由飞书 `生成数量（需填写）` 决定。长视频规格同样只读取飞书 `视频规格（需填写）`，不能由 OpenClaw 临时改写。
...
| 用户自然语言 | 调用 | 行为 |
| --- | --- | --- |
| 检查进入生产的原创脚本 / 看看长视频生产队列 | `check --limit 20` | 只读预览，不生成视频、不写表 |
| 生成勾选的原创视频 / 把勾选脚本送生产 | `sync --limit 20` | 只处理已经勾选“进入生产”的脚本 |
| 继续生成长视频 / 续跑远端长视频 | `sync --record-id rec... --limit 1` | 根据 job 和远端 task ID 幂等续跑 |
| 生成某产品已勾选的原创视频 | `sync --product-code <产品编码>` | 处理该产品已勾选的生产脚本 |
| 发布已完成并勾选确认发布的长视频 | `sync --record-id rec... --limit 1` | 只对已完成且已勾选“确认发布”的长视频执行发布入队 |

同步器会统一分流：15 秒原创进入短视频运行管理表；原创长视频进入 Plan C；20 秒以上复刻进入复刻分段执行器。OpenClaw 不得直接调用某个内部 runner 绕过该分类器。

用户明确说“生成勾选的原创视频”“继续长视频”或“把勾选脚本送生产”，即授权执行相应记录已经表达的 `进入生产` 决策。长视频分段过程中不再追加人工确认。未勾选的记录不执行，OpenClaw 也不能替用户修改为勾选。

“发布长视频”只运行已经勾选 `确认发布` 的记录；自然语言指令本身不替代或改写飞书的发布勾选。当前长视频发布桥接仅支持养号、无挂车语义；带货长视频在发布合同扩展前不得自动入队。

### 执行结果汇报

脚本阶段至少汇报：运营记录 ID、产品编码、视频规格、批次 ID、请求/完成/失败数量，以及长视频 job ID。生产阶段至少汇报：生产脚本记录 ID、实际分流、远端状态、是否 FINAL_READY、成片路径、飞书回写和是否进入发布队列。只读检查明确说明未调用模型、未生成媒体、未写表。

完整架构、状态源和验收说明见：

`/Users/likeu3/.openclaw/workspace/docs/ORIGINAL_CONTENT_SHORT_LONGFORM_HANDOFF_20260909.md`
...
- 长视频人物规划在兼容与穿搭明确关联之后优先已配置多视角图包，再按用量轮换；未开启长视频标记的15秒排序不变。人物参考缓存保留 `is_primary / reference_view / persona_id`，主身份图在同角色内优先，实际生图附件逐张标明商品、人物主图、辅助视角或连续性画面的用途。已有冻结任务不替换人物，缺图包仍允许单图。长视频闭合方式从正向结构描述编译为 `mechanisms`，不再将“单排列数未知”当成“闭合方式未知”，口播不得扫描负向约束来判断拉链存在。已人工确认的纠错可在共享运行数据 `product_identity_corrections.json` 中按产品与完整图片SHA集合精确覆盖；只在新DIRECT_PRODUCT来源构建时应用，不改15秒、原始P1审计、历史job或其他图片版本。详见 `docs/LONGFORM_REFERENCE_IDENTITY_FIX.md`。

- 2026-09-10：长视频主脚本规划（scene_blocks/capture_units）默认 `gpt-6-astra/high`，支持入口 `--blueprint-model` 显式回退Sol/Terra；15秒蓝图仍为Sol。新版只调整写作要求与状态连续性的语义，不增加动作库、数量配额、质量阻断或修订循环。生活连接镜头不用伪装为商品证明；明确切镜允许同一商品/穿搭单品的已规划状态变化，连续边界仍承接实际尾帧。细节取景投影不得覆盖原动作。新稿记录 `longform-writer-v2-product-led-cuts`；已完成稿和resume缓存不自动重写，验证须新建或replan。详见 `docs/LONGFORM_PRODUCT_LED_WRITER.md`。

- 长视频中央口播 `creative_longform_single_v1` 使用 `gpt-6-astra/high`，瞬时失败沿用两次主路由尝试后兜底 `gpt-5.6-sol/high`。使用应用内新版CLI，支持 `LONGFORM_VOICEOVER_CODEX_BIN`；`LONGFORM_VOICEOVER_ASTRA_ENABLED=0` 可回退原Sol→Terra。仅长口播生效，不改变15秒、蓝图、图片和视频模型。实际模型血缘必须保留在口播结果中；旧稿不自动重写。参考样本灰度开关不随模型升级自动开启。详见 `docs/LONGFORM_ASTRA_ROLLOUT.md`。

- 入口：`scripts/run_longform_original.py`；说明：`docs/LONGFORM_PLAN_C_V1.md`。
- 状态仍独立写入 `~/.openclaw/shared/data/longform_original_video.sqlite3`。运营入口复用 `短视频运营任务表`，由 `视频规格（需填写）` 显式选择 `15秒原创 / 20秒 / 25秒 / 30秒 / 35秒 / 40秒 / 45秒`；字段为空按 `15秒原创` 兼容旧任务。只有大于 15 秒时才进入本旁路，15 秒代码路径保持原样。
- 长视频任务的 `生成数量` 单次最多 3 条。运营可选 `长视频场景模式（可选）=自动 / 单场景 / 双场景`；留空等同自动。新任务和replan统一走 `DIRECT_PRODUCT`，先按当前任务附件解析商品锚点，再复用共享PLAN_ONLY分配卖点、结构、钩子、人物、穿搭和场景，不再自动继承近期短脚本的商品颜色与穿搭，不暗中生成15秒脚本。全部来源在模型调用前冻结到 `batches/<batch_id>/source_plan.json`；resume复用原计划，任务图片/国家/语言变化必须replan。CLI显式source-script-id历史实验保持可用，但不是运营默认来源。
- 长视频穿搭新规划启用已有颜色适配字段：读取与同步统一清除产品ID的明确不可见字符；只保守解析suitable_color_rules，颜色偏好只作软排序，不把非同色误当禁用；批内少用优先，同色软优先。复用原穿搭选择器与使用记录，不新增人工字段/轮换系统或近色矩阵；15秒默认不启用颜色参数。完整合同留审计，写作和视频仅投影已选配方、配饰、领口/外层、配色/完成效果。
- 长视频V8不再要求每镜停住；普通切镜可在动作中发生，连续桥接记录实际状态而非强制定格。中央长口播不再要求每段连接句或固定中段四步；输入去掉逐镜解释，保留主情境、购买理由、钩子/话术与分段时长，场景仅为可选背景。直接商品来源补回公共曝光/背景分离，首帧和视频按同一个函数使用本段场景，不继承旧开场动作/全身构图或改写穿搭。已有脚本不自动刷新；新创作验证需新任务或replan。15秒流程、TTS一次实测修订、分段和商品身份锁不变。
- 长视频中央口播V9通过 `central-spoken-writer-v1-concrete-source` 控制写作模型的最终可见输入：同一人工来源先合并，只保留一份具体运营原文及本条选中语义；多主题支持卖点不得把同一编号下的其他主题重新带回。正面展示、背面观察、取景等明确拍摄观察留在视觉审计，不再作为可讲商品事实；具体商品结构仍可讲。口播结构原型与目标语言表面样本独立检索，前两个结构簇缺少合格原句时不再阻断其他合格口语样本；跨钩子人工样本只指导观众关系和论证节奏，跨钩子本土样本只指导语言节奏，均不得改变已冻结钩子或提供商品事实。长口播每段时长改为音频容量而非内容配额，TTS仍只对确实过短/过长的段落最多修订一次；可接受区间内不因更接近93%而替换初稿。完整上游合同继续用于后处理和审计，只有精简投影进入模型；新版本只使口播缓存失效，不改视觉、首帧和15秒默认写作路径。
- 长视频完整案例灰度默认关闭，显式 `LONGFORM_WRITING_CASE_ENABLED=1` 才尝试检索。只有 `reference_status=READY`、`text_extent=FULL` 且正文哈希匹配音频核对/人工文本核对记录的案例才能进入写作模型；旧摘要与省略片段已撤回为待核对候选。选中后模型只见一条完整 reference_text，旧结构原型、关系词配额与人工碎片只留上游审计。无可用案例时正常回退，不影响15秒路径，不增加自动修订。未完成源音频核对不得报告A/B完成或原生表达验证通过。
- 长视频首次口播从中央服务解析完整钩子、批准话术及原生表达资源并冻结central_resource_snapshot，不再仅从旧脚本顶层取空合同。有效音频时长排除明确首尾静音，最终混音复用冻结文件；语义段允许边界附近最多0.75秒微调，不裁句、不强行填满、不增加第二次修订。片段进入帧的场景与取景以本段为权威，长视频近景不强制头到膝部，口型统一服从后配旁白；默认15秒首帧不变。
- 文本完成后，每条长视频在 `原创视频生产脚本` 中形成一行，显示视频规格、分段计划、长视频任务 ID 和执行状态。运营勾选 `进入生产` 后，`script-run-manager-sync` 先把长视频行交给 Plan C 无人工停顿执行器，再把 15 秒行送入原短视频运行管理表；长视频行不得误同步到短视频运行表。Plan C 生成并登记 K0 后，执行器把它一次性回写到同一行的 `长视频首帧（系统）` 附件字段，供运营预览；已有附件时直接复用，展示回写失败只记录提示，不阻断远端生成或成片状态。
...
- 分段口播混流必须先按时间轴定位各段，再对合成后的整条人声统一做响度标准化；禁止在多个输入段分别 `loudnorm` 后直接 `amix`。最终文件和幂等复用都必须通过 `ffprobe` 交付检查：同时存在视频流与音频流、文件大小正常，且时长与合并视频在容差内；未通过不得写 `FINAL_READY` 或上传飞书。
...
- 本旁路只复用两张运营表的显式字段与勾选入口，不得修改现有 15 秒的生成逻辑、默认模型、运行表字段语义或媒体执行链。

## 核心能力

### 当前15秒正式口播路由（2026-09-09覆盖历史模型说明）

`simplified_v1` 正式整段口播由原创桥接携带 `route_scope=original_shortform`，中央 `creative_full_single_v1` 使用 `gpt-6-astra/high`，瞬时失败最多两次主尝试后兜底一次 `gpt-5.6-sol/high`。蓝图仍为Sol，写作规则不变。女装和配饰等共用该桥接的类目同时生效；未标记scope的人工上传/旧分句口播、质检及三候选实验入口不变。`ORIGINAL_SHORTFORM_VOICEOVER_ASTRA_ENABLED=0` 回退短口播原Sol→Terra；长口播独立开关不受影响。实际模型血缘保留，口播缓存识别路由变化但不影响视觉缓存；已完成脚本不自动重写。详见 `docs/ORIGINAL_SHORTFORM_ASTRA_ROLLOUT.md`。
...
- 默认主线路模型配置对齐 OpenClaw 当前主 agent（`openai-codex/gpt-5.6-sol`）
- 当前唯一生效线路：`primary`
- 支持通过命令行显式传入：
  - `--llm-route primary`
...
以下规则只适用于 15 秒任务。`原创视频生产脚本` 以 `生成首帧（需勾选）` 作为唯一人工决策入口。程序不得因为
...
长视频不使用上述人工首帧开关。它在 Plan C 生产过程中自动生成 K0，并回写 `长视频首帧（系统）`；长视频仍直接在本表完成生产和成片回写，不进入短视频自动运行管理表。

也可同时勾选两个开关：总库同步器在原有每六小时巡检中先准备首帧，成功后立即同步同条记录，不新增高频扫描。首帧处理器接收同步器传入的单记录快照，不额外全表读取；已就绪附件直接复用，图片生成成功而上传或回写失败时保留本地图片，只补交接。

成功脚本复刻的 `wsr_` 记录使用总库该行最终视频提示词及人物/商品图片准备首帧，不查原创批次或阶段0结果。首帧只表现该脚本开场的单一静止瞬间，不把后段人物揭晓提前、不生成分镜拼图；养号可没有商品图，不强加商品或带货内容。此适配不改写源脚本、口播或人工勾选。
