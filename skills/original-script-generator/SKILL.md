...
- 文本完成后，每条长视频在 `原创视频生产脚本` 中形成一行，显示视频规格、分段计划、长视频任务 ID 和执行状态。运营勾选 `进入生产` 后，`script-run-manager-sync` 先把长视频行交给 Plan C 无人工停顿执行器，再把 15 秒行送入原短视频运行管理表；长视频行不得误同步到短视频运行表。
