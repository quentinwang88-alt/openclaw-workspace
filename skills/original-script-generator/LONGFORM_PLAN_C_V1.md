# 25–45 秒原创视频可变分段 Plan C（隔离旁路 V3）
...
主合同允许 25–45 秒、一个语义主线、一个开场钩子、2–4 个有角色分工的有效卖点和最多两个场景块。25–30 秒自动切为两段，31–45 秒自动切为三段；每段最长 15 秒且使用整数时长。V6 每段通常保留三个真正可执行的 `OPENING / CORE / PAYOFF` 观看任务，只有确有新证明或新使用关系时才在主合同保留第4个审计单元；H3 不再承担“四个同权镜头必须全部出现”的任务。使用 `plan --use-model` 时复用当前原创模型线路；不带该参数时，可对人工/测试合同做纯确定性编译。

`plan --source-script-id` 会只读中央运营确认卖点库，生成 `longform_argument_bundle`。25–30 秒建议自然使用 2–3 个价值，40–45 秒建议 3–4 个；它们分别承担主结果、商品原因、使用价值或风格回报，禁止机械列举。该容量是软目标，来源不足时不虚构、不阻断。
...
2. `voiceover`：一次生成完整 25–45 秒连续思想及 A/B/C 语义段；后续段不得重新开钩子。
