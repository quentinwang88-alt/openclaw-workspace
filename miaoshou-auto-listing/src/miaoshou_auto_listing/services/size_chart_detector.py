...
只判断整张图片是否主要展示可用于当前商品的服装尺码表、尺码推荐表或 SKU 尺码规格表。
以下都可以视为有效来源：
1. 包含 S/M/L/XL、胸围、衣长、肩宽、腰围、臀围等字段的常规尺码表；
2. 包含“均码/Free size”及建议身高、建议体重或适穿范围的尺码推荐；
3. 1688“包装信息/商品件重尺”中的结构化表格，只要明确含有“尺码”列，并按颜色或 SKU 列出了尺码、长宽高、体积、重量等数据。
第三类表格即使尺寸看起来是包装尺寸也应判定为有效；下游只复用原表，不推测服装尺寸。
