...
class Repository(Protocol):
    ...
    def prompt_exists(self, mother_id: str, mother_version: int, product_id: str, digest: str) -> bool: ...
    def prompt_slot_exists(self, batch_id: str, product_id: str, variant_key: str, mutation_key: str) -> bool: ...
    def prompt_sequence_exists(self, mother_id: str, mother_version: int, product_id: str, sequence_no: int) -> bool: ...
    def list_prompts(self, mother_id: str, mother_version: int, product_id: str) -> list[PromptRecord]: ...
...
class InMemoryRepository:
    ...
    def __init__(self) -> None:
        ...
        self.prompt_digests: set[tuple[str, int, str, str]] = set()
    ...
    def prompt_exists(self, mother_id: str, mother_version: int, product_id: str, digest: str) -> bool:
        with self._lock:
            return (mother_id, mother_version, product_id, digest) in self.prompt_digests
    ...
    def save_prompt(self, record: PromptRecord) -> bool:
        with self._lock:
            digest_key = (record.mother_id, record.mother_version, record.product_id, record.prompt_hash)
            sequence_exists = record.sequence_no > 0 and self.prompt_sequence_exists(
                record.mother_id, record.mother_version, record.product_id, record.sequence_no
            )
    ...
    def enqueue_outbox(self, record: OutboxRecord) -> bool:
        with self._lock:
            if record.outbox_id in self.outbox:
                return False
...
class MySQLRepository:
    ...
    def prompt_exists(self, mother_id: str, mother_version: int, product_id: str, digest: str) -> bool:
        return self._fetchone(
            "SELECT prompt_id FROM wsr_replication_prompt WHERE mother_id=%s AND mother_version=%s AND product_id=%s AND prompt_hash=%s LIMIT 1",
            (mother_id, mother_version, product_id, digest),
        ) is not None
    ...
    def prompt_sequence_exists(self, mother_id: str, mother_version: int, product_id: str, sequence_no: int) -> bool:
        return self._fetchone(
            """SELECT prompt_id FROM wsr_replication_prompt
            WHERE mother_id=%s AND mother_version=%s AND product_id=%s AND sequence_no=%s LIMIT 1""",
    ...
    def list_prompts(self, mother_id: str, mother_version: int, product_id: str) -> list[PromptRecord]:
        with self.connection_factory() as conn, conn.cursor() as cursor:
            cursor.execute(
                ...
                ORDER BY sequence_no,prompt_id""",
                (mother_id, mother_version, product_id),
            )
