...
class FrameSampleSkill:
    ...
    def sample_product(self, product_id: str, force: bool = False, tier: str = "representative") -> Result:
        ...
        if max_workers <= 1 or len(segments) <= 1:
            results = [self.sample_segment(s["segment_id"], force=force, tier=tier).to_dict() for s in segments]
        else:
            results_by_id = {}
            with ThreadPoolExecutor(max_workers=max_workers) as pool:
                futures = {pool.submit(self.sample_segment, s["segment_id"], force, tier): s["segment_id"] for s in segments}
        ...
        return Result.ok({"count": len(results), "tier": tier, "results": results})

    def sample_segment(self, segment_id: str, force: bool = False, tier: str = "representative") -> Result:
        ...
        count = _tier_frame_count(tier, str(segment.get("source_type") or ""))
...
def _tier_frame_count(tier: str, source_type: str) -> int:
    """L1=fingerprint, L2=representative, L3=evidence-intensive."""
    normalized = str(tier or "representative").strip().lower()
    if normalized in {"fingerprint", "l1"}:
        return 1
    if normalized in {"evidence", "l3", "deep"}:
        return 9 if source_type == "ai_generated" else 6
    return 3
