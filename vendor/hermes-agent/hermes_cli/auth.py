...
def normalize_codex_base_url(base_url: str) -> str:
    ...
    OpenClaw stores the canonical Codex base as ``https://chatgpt.com/backend-api``
    and its transport appends ``/codex/responses`` internally. Hermes passes the
    base URL directly to the OpenAI SDK Responses client, so it must end at
    ``.../codex`` instead.
    """
